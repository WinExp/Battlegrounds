...
public record ModVersionConfigurationTask(ServerConfigurationNetworkHandler handler) implements ServerPlayerConfigurationTask {
    public static final Key KEY = new Key(ModVersionPayloadS2C.ID.id().toString());

    @Override
    public void sendPacket(Consumer<Packet<?>> sender) {
        ServerConfigurationNetworking.send(this.handler, new ModVersionPayloadS2C(Constants.MOD_VERSION));
    }
