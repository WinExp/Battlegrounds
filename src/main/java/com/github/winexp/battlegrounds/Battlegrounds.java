...
public class Battlegrounds implements ModInitializer {
    ...
    public static void reload(){
        ...
        if (Variable.INSTANCE.progress.progress.isStarted() && !Variable.INSTANCE.progress.progress.isDeathmatch()){
    ...
    private ActionResult onPlayerJoin(ClientConnection connection, ServerPlayerEntity player, ConnectedClientData clientData, CallbackInfoReturnable<Text> cir){
        if (Variable.INSTANCE.progress.progress.isPreparing()){
            if (!Variable.INSTANCE.progress.players.contains(player.getGameProfile().getId().toString())){
        ...
        else if (Variable.INSTANCE.progress.progress.isDeathmatch()){
