...
public final class ModServerPlayerEvents {
    public static final Event<AllowNaturalRegen> ALLOW_NATURAL_REGEN = EventFactory.createArrayBacked(AllowNaturalRegen.class,
            (listeners) -> (player) -> {
    ...
    public static final Event<AfterDamaged> AFTER_PLAYER_DAMAGED = EventFactory.createArrayBacked(AfterDamaged.class,
            (listeners) -> (player, source, amount) -> {
                for (AfterDamaged listener : listeners) {
                    listener.onPlayerDamaged(player, source, amount);
                }
            });
    public static final Event<AfterJoined> AFTER_PLAYER_JOINED = EventFactory.createArrayBacked(AfterJoined.class,
